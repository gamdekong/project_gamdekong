...
bool Stage1_2::init()
{
	...
	{
		this->schedule(schedule_selector(Stage1_2::tick));
...
bool Stage1_2::createWorld(bool debug)
{
	//월드 생성 시작
...
void Stage1_2::createPlayer(Sprite * player)
{
...
void Stage1_2::createMonster(Sprite * monster)
{
	//--------------플레이어 바디생성
...
void Stage1_2::createBackground()
{
...
void Stage1_2::draw(Renderer * renderer, const Mat4 & transform, uint32_t flags)
{
	//Layer::draw();
...
void Stage1_2::LongAttack(int num)
{
	switch (num)
	{
	case RIGHTLONGATTACK:

		if (!this->isScheduled(schedule_selector(Stage1_2::RightLongAttack)) &&
			!this->isScheduled(schedule_selector(Stage1_2::clearTime)))
		{
			this->RightLongAttack(0);
		}
		else if (!this->isScheduled(schedule_selector(Stage1_2::RightLongAttack)))
			this->scheduleOnce(schedule_selector(Stage1_2::RightLongAttack), player->attackSpeed);
		break;


	case LEFTLONGATTACK:
		if (!this->isScheduled(schedule_selector(Stage1_2::LeftLongAttack)) &&
			!this->isScheduled(schedule_selector(Stage1_2::clearTime)))
		{
			this->LeftLongAttack(0);
		}
		else if (!this->isScheduled(schedule_selector(Stage1_2::LeftLongAttack)))
			this->scheduleOnce(schedule_selector(Stage1_2::LeftLongAttack), player->attackSpeed);
		break;
	}
}

void Stage1_2::RightLongAttack(float dt)
{
	log("오른쪽 검기");
	...
	//joystick2->attack = 0;
	this->scheduleOnce(schedule_selector(Stage1_2::clearTime), player->attackSpeed);

}
void Stage1_2::LeftLongAttack(float dt)
{
	log("왼쪽 검기");
